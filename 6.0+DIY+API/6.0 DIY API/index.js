...
app.get('/filter', (req, res) => {
  ...
  const jokesFiltered = jokes.filter(j => j.jokeType === type);
...
app.delete('/jokes/all', (req, res) => { 
  // se eu coloco esse metodo depois do delete specific joke, ele acha que all é o id
  // colocando antes, ele tenta primeiro acessar esse endpoint e funciona
...
app.delete('/jokes/:id', (req, res) => {
  const id = parseInt(req.params.id);
  const indexToDelete = jokes.findIndex(j => j.id === id);
  if (indexToDelete > -1) {
    ...
    res
      .status(404)
      .send(`Joke with id: ${id} not found. No jokes were deleted`);
  };
})
